...
msWorkspace.RegisterWorkspaceFailedHandler(MsWorkspace_WorkspaceFailed);
var solution = await msWorkspace.OpenSolutionAsync(solutionFile);

var docs = solution
    .Projects.Where(projet => projet.AssemblyName.EndsWith("Implementation"))
    .SelectMany(p => p.Documents)
    .ToList();

await Task.WhenAll(
    docs.Select(async doc =>
    {
        ...
        var testProject = solution.Projects.SingleOrDefault(p =>
            p.Name == $"{doc.Project.Name}.Test"
        );
        if (testProject == null)
        {
            return;
        ...
        foreach (var methDecl in classDecl.DescendantNodes().OfType<MethodDeclarationSyntax>())
        {
            ...
            var (fileName, folder, content) = TestGenerator.GenerateTest(
                methSymbol,
                classDecl,
                DalTestStrategy.Semantic
            );
...
static void MsWorkspace_WorkspaceFailed(WorkspaceDiagnosticEventArgs e)
{
